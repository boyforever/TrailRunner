...
var jsonsql = {
	query: function(sql, json) {
		const returnfields = sql.match(/^(select)\s+([a-z0-9_,.\s*]+)\s+from\s+([a-z0-9_.]+)(?: where\s+\((.+)\))?\s*(?:order\sby\s+([a-z0-9_,]+))?\s*(asc|desc|ascnum|descnum)?\s*(?:limit\s+([0-9_,]+))?/i)
		const ops = {
	...
	returnFields: function(scope, fields) {
		...
		var returnobj = {}
		for (const i in fields) returnobj[fields[i]] = scope[fields[i]]
